...
class ImageCompressionHelper {
  ...
  static Future<File> compressIfNeeded(File file, {int maxSizeInBytes = 500 * 1024, int targetDimension = 800}) async {
    try {
      ...
      debugPrint('Compressing file: ${file.path} (${(fileSize / 1024).toStringAsFixed(1)} KB)');
      ...
      final tempFile = File('${Directory.systemTemp.path}/compressed_${DateTime.now().millisecondsSinceEpoch}.$extension');
      await tempFile.writeAsBytes(compressedBytes);
      
      debugPrint('Compression finished: ${tempFile.path} (${(compressedBytes.length / 1024).toStringAsFixed(1)} KB)');
      return tempFile;
    } catch (e) {
