...
class _StyleDnaChatPageState extends State<StyleDnaChatPage> {
  ...
  Future<void> _loadInitialData() async {
    ...
    final name =
        ...
        'bạn';
    final helloMessage =
        'Xin chào $name! Tôi là AI Stylist cá nhân của bạn. Dựa trên hồ sơ chiều cao, cân nặng, tông da và tủ đồ thực tế của bạn, tôi sẽ giúp bạn thiết kế những bộ cánh thời trang và cá nhân hóa nhất. Hôm nay bạn muốn mặc đồ đi đâu hay có câu hỏi gì cho tôi không?';

    setState(() {
      _messages.add({'role': 'model', 'text': helloMessage});
      _isLoading = false;
      _isInitLoaded = true;
    });
  }
  ...
  Future<void> _sendMessage(String text) async {
    ...
    final suggestedColors = personalColorProfile.bestColors
    ...
    try {
      final response = await _geminiService.chatStyle(
        ...
        heightCm: widget.profileData['heightCm'] != null
            ? double.tryParse(widget.profileData['heightCm'].toString())
            : null,
        weightKg: widget.profileData['weightKg'] != null
            ? double.tryParse(widget.profileData['weightKg'].toString())
            : null,
        country:
            widget.profileData['country']?.toString() ??
            widget.profileData['Country']?.toString(),
      ...
      if (response != null) {
        ...
        setState(() {
          _messages.add({
            'role': 'model',
            'text':
                'Rất tiếc, tôi đang gặp lỗi kết nối với máy chủ AI. Bạn hãy thử lại sau nhé!',
          });
        });
      }
    } catch (e) {
      setState(() {
        _messages.add({
          'role': 'model',
          'text':
              'Đã xảy ra lỗi không mong muốn. Xin lỗi bạn vì sự bất tiện này.',
        });
      });
    } finally {
  ...
  Widget build(BuildContext context) {
    final height =
        widget.profileData['heightCm']?.toString() ??
        widget.profileData['HeightCm']?.toString();
    final weight =
        widget.profileData['weightKg']?.toString() ??
        widget.profileData['WeightKg']?.toString();
    ...
    final List<String> suggestions = [
      ...
      if (height != null && weight != null)
        'Tư vấn phối đồ cho người cao $height cm',
...
class MarkdownText extends StatelessWidget {
  ...
  const MarkdownText({super.key, required this.text, required this.style});
