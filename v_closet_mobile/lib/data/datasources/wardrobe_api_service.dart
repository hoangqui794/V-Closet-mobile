...
class WardrobeApiService {
  ...
  Future<List<ClothingItem>> getItems({String? category, String? color}) async {
    try {
      ...
      final response = await _apiService.get('/Wardrobe', queryParameters: queryParams);
  ...
  Future<ClothingItem?> getItem(String id) async {
    try {
      final response = await _apiService.get('/Wardrobe/$id');
      if (response.statusCode == 200) {
  ...
  Future<ClothingItem?> uploadAndCreateItem({
    ...
    try {
      ...
      final response = await _apiService.post('/Wardrobe/upload-and-create', data: formData);
  ...
  Future<ClothingItem?> updateItem(String id, Map<String, dynamic> data) async {
    try {
      final response = await _apiService.put('/Wardrobe/$id', data: data);
      if (response.statusCode == 200) {
        return ClothingItem.fromJson(response.data);
      }
      return null;
  ...
  Future<bool> deleteItem(String id) async {
    try {
      final response = await _apiService.delete('/Wardrobe/$id');
